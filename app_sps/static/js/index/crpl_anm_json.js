...
const toastAnimation = lottie.loadAnimation({
    container: toastAnimationContainer,
    renderer: 'svg',
    loop: false,
    autoplay: false,
...
function showToast(message, callback, animate = true) {
    ...
    if (animate) {
        toastAnimationContainer.style.display = 'block';
        toastAnimation.play();
    } else {
        toastAnimationContainer.style.display = 'none';
    }
...
document.getElementById('create-playlist-form').addEventListener('submit', async (event) => {
    ...
    const messageList = JSON.parse(dataInput.getAttribute('data-createpl-list') || '[]');
